...
Both are started by systemd in normal operation. `ground_station` has flown on
Ekko; `ocs_client` has run aboard but has not been checked against RoboNation's
stub.
...
| Map | **altitude in big type on the map**, green up to where climbs stop (`FENCE_ALT_MAX − FENCE_MARGIN`), amber above, red within 0.5 m of `FENCE_ALT_MAX`; beside the map **a 3D Ekko** — its own Onshape assembly, cut to ~2500 triangles by `tools/scripts/make_ekko_model.py` into `ekko_model.py` (re-run it when the CAD changes; the GLB stays out of the repo) — turned, rolled and pitched as the autopilot reports it (20 Hz from `GET /attitude`), roll / pitch / heading, a tilt warning (amber 20°, red 30°) and 10 s of roll and pitch; aircraft, trail, **the fence the autopilot holds**, inside/outside, GPS quality, a grid that re-spaces itself with zoom, **the camera's footprint**, the Task 1 buoys with their states, and **the buoy search**: its path, the shrunk fence, where it is heading, the buoys it is hovering over or skipped | pan, zoom, **fit** (frames the fence, buoys and boat; done by itself when a fence is read), follow (pans only when the aircraft nears an edge), clear trail, clear buoys, download the map, **measure** between two points or buoys, **beep** when a buoy locks; **search on/off and buoys to find** — neither can start a flight |
